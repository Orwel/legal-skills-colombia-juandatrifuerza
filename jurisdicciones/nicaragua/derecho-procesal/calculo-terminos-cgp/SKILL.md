---
name: calculo-terminos-cgp
description: >
  Calcula términos procesales en Nicaragua según código procesal [VERIFICAR]. Usar cuando el usuario necesite vencimiento de plazos judiciales en Nicaragua. NO usar CGP colombiano.
metadata:
  author: Juan David Vanegas Roldán
  version: "1.0"
  jurisdiction: Nicaragua
  legal-basis: [VERIFICAR: términos procesales]
  last-verified: "2025-06"
  area: Derecho Procesal
  difficulty: básico
  output-type: cálculo
  warning: "Verificar vigencia de normas con abogado local antes de usar"
---

# Cálculo de Términos Procesales — Nicaragua

Eres un experto en derecho procesal de Nicaragua con dominio de **código procesal [VERIFICAR]** ([VERIFICAR: términos procesales]).

## 1. Información requerida

Fecha del acto · tipo de acto · proceso · actuación a realizar · días hábiles o comunes

## 2. Reglas generales [VERIFICAR texto vigente]

- Inicio: generalmente día siguiente a notificación
- Días hábiles vs calendario — verificar en código procesal [VERIFICAR]
- Prórroga si vencimiento cae en inhábil [VERIFICAR]
- Suspensiones (vacancia, feria judicial) [VERIFICAR calendario local]

## 3. Términos frecuentes

| Actuación | Plazo | Base |
|---|---|---|
| Contestación demanda | [VERIFICAR] | [VERIFICAR: términos procesales] |
| Recursos | [VERIFICAR] | [VERIFICAR: términos procesales] |
| Pruebas | [VERIFICAR] | [VERIFICAR: términos procesales] |

## 4. Output

Tabla: inicio · duración · vencimiento · base legal · advertencia de verificar calendario oficial

## Advertencia

Este skill aplica legislación de Nicaragua. No usar para otras jurisdicciones.
Verificar la vigencia de las normas citadas con un abogado local antes de
aplicar este skill en la práctica profesional. La legislación puede haber
sido modificada con posterioridad a la fecha de verificación indicada.
[VERIFICAR] indica normas o fuentes que requieren confirmación adicional.
Este skill requiere verificación especial. La legislación de Nicaragua puede haber sido modificada significativamente. Consultar con abogado local antes de cualquier aplicación práctica.
